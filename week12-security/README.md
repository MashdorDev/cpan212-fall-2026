...
- **`upgrade-insecure-requests` only in production.** It tells the browser to switch `http://` loads to `https://`. On `http://localhost` there is no HTTPS, and Safari can then fail to load the admin CSS and JavaScript.
