...
export default function NewEventPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // The browser attaches the session cookie by itself, because the request is to the same origin.
      ...
      // proxy.js only checked that a cookie exists. If the session behind it expired or was logged out
      // somewhere else, the API is the one that finds out.
      if (res.status === 401) {
        setUser(null);
        setLoggedOut(true);
        setMessage(body.error?.message ?? 'Log in to do this');
        return;
      }

      // fetch does not throw for a 400 or 500, so check res.ok before treating the response as a success.
      if (!res.ok) {
    ...
    <>
      ...
      {message && (
        <div className={styles.summary} role="alert">
          ...
          {loggedOut && (
            <p>
              Your session has ended. <Link href="/login?next=/events/new">Log in again</Link>, then create the event.
            </p>
          )}
        </div>
      )}
