// Week 6 has no API yet, so the events live in this file. In Week 7 these two functions
// call the Express API instead, and the pages that use them don't need to change.
...
// Makes the list slow on purpose so you can see app/events/(list)/loading.js while running `npm run dev`.
const FAKE_DELAY_MS = 600;

export async function getEvents() {
  await new Promise((resolve) => setTimeout(resolve, FAKE_DELAY_MS));
  return events.toSorted((a, b) => a.startsAt.localeCompare(b.startsAt));
}
