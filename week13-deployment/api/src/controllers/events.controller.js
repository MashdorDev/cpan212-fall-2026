...
// Only these fields are copied from the request body. Anything else, like _id, createdAt or organizer,
// is ignored, so a client can't choose its own id, backdate an event or create one in someone else's name.
...
export async function createEvent(req, res) {
  ...
  const event = await Event.create({ ...pickEditableFields(req.body), organizer: req.user._id });
...
export async function updateEvent(req, res) {
  ...
  // Load the event first to check who owns it: 404 if it doesn't exist, 403 if it isn't yours.
  const event = await findByIdOr404(Event, req.params.id);
  ...
  const updated = await Event.findByIdAndUpdate(event._id, changes, { returnDocument: 'after', runValidators: true });
